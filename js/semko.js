...
ko.utils = new (function () {
    ...
    return {
        ...
        evalWithinScope: function (expression, scope, node) {
          ...
          return (new Function("__SKO__sc", "with(__SKO__sc){ var innerNode=skonode; console.log('NOW!!');  var res = (" + expression + "); console.log(res); return res; }"))(scope);
        },
...
ko.jsonExpressionRewriting = (function () {
	...
	parseURIsInJSONString: function(jsonString) {
		...
		value = this.parseURIsInJSONString(value);
                ...
                if(value[0]==='<' && value[value.length-1]==='>' && key !== 'about' && key !== 'rel') {
                    readers = readers+key+": (function(){ sko.current = function() { return sko.currentResource(innerNode); }; console.log(1); return sko.current().tryProperty('"+value+"') })()";
                } else if(value.match(/^\[[^,;"\]\}\{\[\.:]+:[^,;"\}\]\{\[\.:]+\]$/) != null && key !== 'about' && key !== 'rel') {
                    readers = readers+key+": (function(){ sko.current = function() { return sko.currentResource(innerNode); }; console.log(2); return sko.current().tryProperty('"+value+"') })()";
                } else if(value[0]==="<" && value[value.length-1]===">" && (key === 'about' || key === 'rel')) {
...
sko.about = function(aboutValue, viewModel, cb) {
    ...
    if(currentValue != null) {
        ...
        if(typeof(aboutValue) === 'string') {
          ...
          sko.store.node(sko.plainUri(uri), function(err, resource) {
                if(!err) {
                    ...
                    var subscription = sko.about[nextId].subscribe(function(nextUri) {
                        ...
                        sko.store.node(sko.plainUri(nextUri), function(err, nextResource) {
                            if(!err) {
            ...
            // register the new observer and resource
            sko.store.node(sko.plainUri(sko.about[nextId]()), function(err, resource) {
                if(!err) {
                    // id -> Resource
                    ...
                    if(resource[uri]) {
                        ...
                        if(relResourceUri != null && !sko.isSKOBlankNode(resource[uri]())) {
                            ...
                            if(sko.aboutResourceMap[nextId] == null || sko.aboutResourceMap[nextId].about() != relResourceUri) {
                                ...
                                // register the new observer and resource
                                sko.store.node(sko.plainUri(relResourceUri), function(err, resource) {
